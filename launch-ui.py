...
print(
    f"default encoding is {sys.getdefaultencoding()},file system encoding is {sys.getfilesystemencoding()}"
)
...
if not os.path.exists(os.path.join("./checkpoints/", "vallex-checkpoint.pt")):
    ...
    try:
        ...
        wget.download(
            ...
            out="./checkpoints/vallex-checkpoint.pt",
...
def main():
    ...
    with app:
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    textbox_transcript = gr.TextArea(
                        ...
                        value="",
                        elem_id=f"prompt-name",
                    )
                    upload_audio_prompt = gr.Audio(
                        label="uploaded audio prompt", source="upload", interactive=True
                    )
                    record_audio_prompt = gr.Audio(
                        label="recorded audio prompt",
                        source="microphone",
                        interactive=True,
                    )
                with gr.Column():
        ...
        with gr.Tab("Make prompt"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    upload_audio_prompt_2 = gr.Audio(
                        label="uploaded audio prompt", source="upload", interactive=True
                    )
                    record_audio_prompt_2 = gr.Audio(
                        label="recorded audio prompt",
                        source="microphone",
                        interactive=True,
                    )
                with gr.Column():
                    ...
                    btn_2.click(
                        ...
                        inputs=[
                            textbox2,
                            upload_audio_prompt_2,
                            record_audio_prompt_2,
                            textbox_transcript2,
                        ],
                        outputs=[text_output_2, prompt_output_2],
                    )
            gr.Examples(
                examples=make_npz_prompt_examples,
                inputs=[
                    textbox2,
                    upload_audio_prompt_2,
                    record_audio_prompt_2,
        ...
        with gr.Tab("Infer from prompt"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    language_dropdown_3 = gr.Dropdown(
                        choices=["auto-detect", "English", "中文", "日本語", "Mix"],
                        value="auto-detect",
                        label="language",
                    )
                    accent_dropdown_3 = gr.Dropdown(
                        choices=["no-accent", "English", "中文", "日本語"],
                        value="no-accent",
                        label="accent",
                    )
                    preset_dropdown_3 = gr.Dropdown(
                        choices=preset_list, value=None, label="Voice preset"
                    )
                    prompt_file = gr.File(
                        file_count="single", file_types=[".npz"], interactive=True
                    )
                with gr.Column():
            ...
            gr.Examples(
                examples=infer_from_prompt_examples,
                inputs=[
                    textbox_3,
                    language_dropdown_3,
                    accent_dropdown_3,
        ...
        with gr.Tab("Infer long text"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    language_dropdown_4 = gr.Dropdown(
                        choices=["auto-detect", "English", "中文", "日本語"],
                        value="auto-detect",
                        label="language",
                    )
                    accent_dropdown_4 = gr.Dropdown(
                        choices=["no-accent", "English", "中文", "日本語"],
                        value="no-accent",
                        label="accent",
                    )
                    preset_dropdown_4 = gr.Dropdown(
                        choices=preset_list, value=None, label="Voice preset"
                    )
                    prompt_file_4 = gr.File(
                        file_count="single", file_types=[".npz"], interactive=True
                    )
                with gr.Column():
