...
def collate(batch):
    ...
    audio_features_s = (
        torch.zeros([len(batch), max(audio_features_lens_s), 8], dtype=torch.int64) - 1
    )  # audio pad with -1

    text_tokens_lens_s = [b["text_tokens_lens"] for b in batch]
    # create an empty tensor with maximum text tokens length
    text_tokens_s = (
        torch.zeros([len(batch), max(text_tokens_lens_s)], dtype=torch.int64) + 3
    )  # [PAD] token id 3
    ...
    for i, b in enumerate(batch):
        ...
        audio_features_s[i, :audio_features_lens, :] = torch.LongTensor(
            audio_features.T
        )
...
def create_dataloader(
    ...
    train_dataset = AudioDataset(
        h5_path=f"{data_dir}/audio_sum.hdf5",
